package com.FoodBoxs.android.adapter;
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof mainDataViewHolder) {
            ...
            if (dataList.get(position).getItemToppings() != null) {
                for (CartItemTopping e : toppings) {
                    toppingsList.append(e.getToppingName()).append(", ");
                }
                viewHolderOrderDetail.tv_subItem_list.setText(toppingsList.toString().substring(0, toppingsList.toString().length() - 2));

            } else viewHolderOrderDetail.tv_subItem_list.setText(context.getString(R.string.no_ingredients));
